...
<div class="d-flex justify-content-between gap-3 mb-4 page-heading">
  <div><h1 class="h2 mb-1">Resumen de plataforma</h1><p class="text-muted mb-0">Estado actual de usuarios, vitrinas, licencias y pagos.</p></div>
</div>
<div class="row g-3 mb-4">
  <?php foreach ([
    ['Usuarios', $stats['users']['total'], $stats['users']['verified'] . ' verificados'],
    ['Vitrinas', $stats['sites']['configured'], ($stats['sites']['published'] ?? 0) . ' publicadas'],
...
<div class="row g-4">
  ...
  <div class="col-lg-5"><section class="card admin-card h-100"><div class="card-body"><h2 class="h5">Estados a vigilar</h2><ul class="list-group list-group-flush"><li class="list-group-item d-flex justify-content-between">En gracia <strong><?= (int)($stats['subscriptions']['grace_period'] ?? 0) ?></strong></li><li class="list-group-item d-flex justify-content-between">Vencidas <strong><?= (int)($stats['subscriptions']['expired'] ?? 0) ?></strong></li><li class="list-group-item d-flex justify-content-between">Vitrinas pausadas <strong><?= (int)($stats['sites']['paused'] ?? 0) ?></strong></li><li class="list-group-item d-flex justify-content-between">Pausadas por vencimiento <strong><?= (int)$stats['sites']['paused_expired'] ?></strong></li><li class="list-group-item d-flex justify-content-between">Pagos rechazados <strong><?= (int)($stats['payments']['rejected'] ?? 0) ?></strong></li></ul></div></section></div>
