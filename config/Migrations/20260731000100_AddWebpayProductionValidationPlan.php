...
class AddWebpayProductionValidationPlan extends BaseMigration
{
    public function up(): void
    {
        ...
        $this->execute(
            "INSERT INTO plans (name, slug, monthly_price, annual_price, max_sites, max_published, sort_order, active, commercial_description, commercial_badge, annual_benefits, capabilities, created, modified)
             VALUES ('Validación productiva Webpay', 'webpay-production-validation', 50, NULL, 0, 0, 1000, false, 'Orden interna de $50 para validar Webpay Plus en producción.', NULL, '[]', '{$capabilities}', '{$now}', '{$now}')
    ...
    {
        $this->execute("DELETE FROM plans WHERE slug = 'webpay-production-validation'");
    }
}
